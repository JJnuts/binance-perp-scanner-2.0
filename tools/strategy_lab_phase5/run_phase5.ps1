...
function Assert-ChildPath {
    ...
    if (-not $Child.StartsWith($prefix, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Refusing filesystem operation outside Phase 5 root: $Child"
    }
}
...
Push-Location $root
try {
    Write-Host "`n== Phase 5 validation pipeline ==" -ForegroundColor Cyan
    docker compose run --rm validation-proof
    if ($LASTEXITCODE -ne 0) {
        throw "Phase 5 validation failed with exit code $LASTEXITCODE"
    }
    Write-Host "`nPhase 5 runtime proof passed." -ForegroundColor Green
