...
function Assert-ChildPath {
    ...
    if (-not $Child.StartsWith($prefix, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Refusing filesystem operation outside Phase 8 root: $Child"
    }
}
...
$baseImageAvailable = $LASTEXITCODE -eq 0
$ErrorActionPreference = $previousErrorActionPreference
if (-not $baseImageAvailable) {
    throw @"
...
Then return to Phase 8 and rerun this script.
"@
}
...
Push-Location $root
try {
    Write-Host "`n== Build pinned Phase 8 Streamlit image ==" -ForegroundColor Cyan
    docker compose build ui-proof
    if ($LASTEXITCODE -ne 0) {
        throw "Phase 8 image build failed with exit code $LASTEXITCODE"
    }

    Write-Host "`n== Phase 8 Streamlit and job-lifecycle proof ==" -ForegroundColor Cyan
    docker compose run --rm ui-proof
    if ($LASTEXITCODE -ne 0) {
        throw "Phase 8 runtime proof failed with exit code $LASTEXITCODE"
    }
    Write-Host "`nPhase 8 runtime proof passed." -ForegroundColor Green
